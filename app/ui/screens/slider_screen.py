...
class SliderScreen:
    ...
    def show(self):
        ...
        self.box_width = 720
        self.box_height = 490
        text_box_width = 300 if self.type == "cover" else 130
        text_box_height = 100
        ...
        self.text_box = self.canvas.create_rectangle(
            math.ceil((self.box_width - text_box_width) / 2),
            math.ceil((self.box_height - text_box_height) / 2),
            self.box_width - math.ceil((self.box_width - text_box_width) / 2),
            self.box_height - math.ceil((self.box_height - text_box_height) / 2),
            fill="#000000",
            width=0,
        )

        self.value_text = self.canvas.create_text(
            math.ceil(self.box_width / 2),
            math.ceil(self.box_height / 2),
            text="",
            font=("Helvetica", 40),
            fill="white",
        )
        ...
        image_path = os.fspath(IMAGES_DIR / "buttons" / "back-white.png")
    ...
    def interpolate_color(self, input_value):
        ...
        interpolated_rgb = tuple(
            int(start_rgb[i] + (end_rgb[i] - start_rgb[i]) * (input_value / 100.0))
            for i in range(3)
        )
    ...
    def close_timer(self, startnew=True):
        ...
        if startnew:
            self.close_timeout = self.window.after(
                self.main_app.settings.destroy_slider_timeout,
                self.destroy_slider,
            )
